...
"""
AI Agent API Routes - Gemini-powered analysis endpoints
"""
...
def get_performance_analysis():
    """GET /api/ai-agent/analysis - Get Gemini-powered performance analysis"""
    try:
        user = get_user_from_token()
        if not user or "id" not in user:
            return jsonify({"success": False, "error": "Authentication required"}), 401

        print(f"[API] Fetching Gemini analysis for user: {user['id']}")
        ...
        if not analysis:
            return jsonify({
                "success": True,
                "data": None,
                "message": "No quiz attempts yet - start taking quizzes to get AI analysis"
            })
...
def get_weak_topics():
    """GET /api/ai-agent/weak-topics - Get AI-identified weak topics"""
    try:
        user = get_user_from_token()
        if not user or "id" not in user:
            return jsonify({"success": False, "error": "Authentication required"}), 401

        print(f"[API] Fetching AI-identified weak topics for user: {user['id']}")
...
def run_agent_cycle():
    """POST /api/ai-agent/run-cycle - Trigger Gemini-powered AI agent cycle"""
    try:
        user = get_user_from_token()
        if not user or "id" not in user:
            return jsonify({"success": False, "error": "Authentication required"}), 401

        print(f"[API] Running Gemini-powered agent cycle for user: {user['id']}")
...
def get_scheduled_tests():
    ...
    try:
        ...
        print(f"[API] Fetching AI-scheduled tests for user: {user['id']}")
